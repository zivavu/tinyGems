          ...
          <PopoverPanel className="absolute z-10 mt-2 w-72 bg-gray-900 rounded-lg border border-gray-800 shadow-lg">
            <div className="p-3">
              <div className="flex justify-between items-center mb-3">
                ...
                {selected.length > 0 && (
                  <Button onClick={() => setSelected([])} className="text-xs text-gray-400 transition-colors hover:text-gray-200">
                    Clear all
                  </Button>
                )}
              </div>
                ...
                {options.map((option) => (
                  <Button
                    ...
                    className={cn(
                      ...
                      selected.includes(option.id) ? 'bg-rose-500/20 text-rose-400' : 'text-gray-300 hover:bg-gray-800',
                    )}
                  >
                    <span>{option.label}</span>
                    {option.description && (
                      <span className={cn('text-xs', selected.includes(option.id) ? 'text-rose-300/70' : 'text-gray-500')}>
                        {option.description}
                      </span>
                    )}
                  </Button>
