import { ArtistMatch, MatchedArtist, MatchedPlatformArtists } from '@/features/artists/types';
...
// Define the ConnectedPlatformsRecord type within this file
...
export function ConnectPlatformsStep({ artistData, onPrevious, onComplete, sourcePlatform }: ConnectPlatformsStepProps) {
  ...
  // Ensure we have valid artist data with a name
  const hasValidArtistName = !!artistData?.name && typeof artistData.name === 'string' && artistData.name.trim() !== '';

  // Use sourcePlatform if provided, otherwise try to determine from links
  const currentPlatform = sourcePlatform || (Object.keys(artistData?.links || {})[0] as PlatformType);
  ...
  const findAcrossPlatformsQuery = trpcReact.externalArtistDataRouter.findAcrossPlatforms.useQuery(
    ...
    {
      enabled: hasValidArtistName, // Only run the query if we have a valid artist name
  ...
  function handleSearch() {
    if (!hasValidArtistName) {
      return; // Don't allow search without a valid artist name
    }
    setIsSearching(true);
  ...
  function handleComplete() {
    ...
    // Create a connected platforms record to pass to the parent component
    const platformLinks: ConnectedPlatformsRecord = {};

    // Add the current platform that we started with
    if (currentPlatform) {
      platformLinks[currentPlatform] = {
    ...
    // Add matches from search results
      ...
      for (const platformMatch of match.platformMatches) {
        ...
        if (artistId) {
          ...
          if (selectedArtist) {
            ...
            // Add to platformLinks record
            platformLinks[platform] = {
              name: selectedArtist.artistName,
              platformId: selectedArtist.artistId,
              avatar: selectedArtist.thumbnailImageUrl || undefined,
            };
          }
        }
      }
    ...
    for (const [platform, url] of Object.entries(manualLinks)) {
      if (url) {
        ...
        // Add to platformLinks record for manual links
        if (!platformLinks[platform]) {
          platformLinks[platform] = {
            name: artistData.name, // Use the artist name since we don't have specific data for manual links
            platformId: `manual-${platform}-${Date.now()}`, // Generate a unique ID
            // No avatar for manual links
          };
        }
      }
    }

    onComplete(mergedData, platformLinks);
  }
  ...
  // Display an error message if no artist name is provided
  if (!hasValidArtistName) {
    return (
      <div className="space-y-6">
        <div className="space-y-2">
          <Typography variant="h3">Connect Artist to More Platforms</Typography>
          <Typography variant="muted">Connect this artist to other music platforms to show all their music in one place.</Typography>
        </div>
        ...
        <div className="p-4 bg-red-50 dark:bg-red-950/30 rounded-lg border border-red-200 dark:border-red-800 text-center">
          <Typography variant="small" className="text-red-700 dark:text-red-400">
            Error: Artist name is required to search across platforms.
          </Typography>
        </div>
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                ...
                <div className="space-y-3">
                  ...
                  {matches.flatMap((match) =>
                    match.platformMatches
                      .filter((pm: MatchedPlatformArtists) => pm.platform === 'soundcloud' && pm.possibleArtists?.length)
                      .flatMap(
                        (pm: MatchedPlatformArtists) =>
                          pm.possibleArtists
                            ?.sort((a: MatchedArtist, b: MatchedArtist) => b.confidence - a.confidence)
                            .slice(0, 3)
                            .map((artist: MatchedArtist) => (
                              <ArtistMatchCard
                                key={artist.artistId}
                                artist={artist}
                                platform="soundcloud"
                  ...
                  {!matches.some((match) =>
                    match.platformMatches.some((pm: MatchedPlatformArtists) => pm.platform === 'soundcloud' && pm.possibleArtists?.length),
                  ) && (
              ...
              {!connectedPlatforms.has('youtube') && (
                <div className="space-y-3">
                  ...
                  {matches.flatMap((match) =>
                    match.platformMatches
                      .filter((pm: MatchedPlatformArtists) => pm.platform === 'youtube' && pm.possibleArtists?.length)
                      .flatMap(
                        (pm: MatchedPlatformArtists) =>
                          pm.possibleArtists
                            ?.sort((a: MatchedArtist, b: MatchedArtist) => b.confidence - a.confidence)
                  ...
                  {!matches.some((match) =>
                    match.platformMatches.some((pm: MatchedPlatformArtists) => pm.platform === 'youtube' && pm.possibleArtists?.length),
                  ) && (
                    <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-lg border border-dashed border-gray-300 dark:border-gray-700 text-center">
                      <Typography variant="small" className="text-gray-500 dark:text-gray-400">
              ...
              {!connectedPlatforms.has('tidal') && (
                <div className="space-y-3">
                  ...
                  {matches.flatMap((match) =>
                    match.platformMatches
                      .filter((pm: MatchedPlatformArtists) => pm.platform === 'tidal' && pm.possibleArtists?.length)
                      .flatMap(
                        (pm: MatchedPlatformArtists) =>
                          pm.possibleArtists
                            ?.sort((a: MatchedArtist, b: MatchedArtist) => b.confidence - a.confidence)
                  ...
                  {!matches.some((match) =>
                    match.platformMatches.some((pm: MatchedPlatformArtists) => pm.platform === 'tidal' && pm.possibleArtists?.length),
                  ) && (
                    <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-lg border border-dashed border-gray-300 dark:border-gray-700 text-center">
                      <Typography variant="small" className="text-gray-500 dark:text-gray-400">
              ...
              {!connectedPlatforms.has('spotify') && (
                <div className="space-y-3">
                  ...
                  {matches.flatMap((match) =>
                    match.platformMatches
                      .filter((pm: MatchedPlatformArtists) => pm.platform === 'spotify' && pm.possibleArtists?.length)
                      .flatMap(
                        (pm: MatchedPlatformArtists) =>
                          pm.possibleArtists
                            ?.sort((a: MatchedArtist, b: MatchedArtist) => b.confidence - a.confidence)
                  ...
                  {!matches.some((match) =>
                    match.platformMatches.some((pm: MatchedPlatformArtists) => pm.platform === 'spotify' && pm.possibleArtists?.length),
                  ) && (
                    <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-lg border border-dashed border-gray-300 dark:border-gray-700 text-center">
                      <Typography variant="small" className="text-gray-500 dark:text-gray-400">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {Object.entries(selectedMatches).map(([platform, artistId]) => {
                const artist = matches
                  .flatMap((m) =>
                    m.platformMatches
                      .filter((pm: MatchedPlatformArtists) => pm.platform === platform)
                      .flatMap((pm: MatchedPlatformArtists) => pm.possibleArtists || []),
                  )
