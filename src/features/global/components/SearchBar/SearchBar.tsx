...
export function SearchBar() {
  ...
  function handleSelect(result: SearchResult) {
    ...
    router.push(`${routes[result.type]}/${result.item.id}`);
  }
    ...
    <div className="w-full">
      <Combobox onChange={handleSelect}>
        <div className="relative">
          ...
          >
            <Combobox.Options className="absolute z-50 mt-2 w-full overflow-hidden rounded-xl bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none dark:bg-gray-900 dark:ring-white/10">
                  ...
                  <Combobox.Option
                    ...
                    className={({ active }) =>
                      `relative cursor-pointer select-none px-4 py-2 ${
                        active ? 'bg-rose-50 dark:bg-rose-900/30' : ''
                      }`
                    }
                  >
                    {({ active }) => (
                        ...
                        <div>
                          <Typography
                            ...
                            className={`font-medium ${
                              active ? 'text-rose-800 dark:text-rose-200' : 'text-gray-900 dark:text-gray-100'
                            }`}
                          >
                            {getItemTitle(result)}
                          </Typography>
